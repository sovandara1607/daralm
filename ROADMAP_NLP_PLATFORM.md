...
3. ⚠️ **Grammar/spelling correction — done, a real bug found+fixed, then a real nuanced result.** `scripts/prepare_grammar_dataset.py` + `scripts/train_sft.py` (configs/50m-grammar.yaml), 400 SFT steps, training perplexity improved steadily (60.5→49.7). First real generation-based evaluation (`scripts/evaluate_grammar.py`) looked like total failure (CER 1.06-1.25 vs a 0.028 no-op baseline) — but a memorization diagnostic (`scripts/overfit_test_grammar.py`) traced this to a real bug in `generate_chat`'s stop condition (comparing against the literal `"</assistant>"` string, which this tokenizer's `<`/`>` coverage gap means never actually appears in decoded output), now fixed in `daralm/inference/generator.py`. With the fix: the model memorizes its training set well (13/16 exact matches on a tiny fixed set) but still does **not** generalize to unseen corrupted sentences (held-out CER still 1.23 vs 0.028 baseline, confirmed after the fix, not a bug artifact). Corrected conclusion: the SFT recipe *can* learn this task's mapping, it just needs more data/steps than 400 steps on 3000 examples to generalize — a real, different, more actionable finding than "generation is fundamentally broken." Full writeup in `README.md`.
4. **Structured JSON generation** — the explicit go/no-go gate before NER, tool calling, or NL-to-SQL are attempted at all. **Informed by item 3's corrected result**: don't assume automatic failure — the recipe demonstrably *can* learn generation-family tasks given enough signal (proven by the memorization diagnostic) — but do expect the same generalization gap unless given a meaningfully larger training budget (more than 400 steps / 3000 examples) than item 3 used, and validate with the same memorize-first-then-generalize diagnostic pattern before trusting a held-out result either way.
5. Translation (only after sourcing and license-vetting a real parallel corpus)

**Stage 3 — only if Stage 2's structured-output gate passes with real measured reliability**
...
**Next up: Stage 2, item 3 — grammar/spelling correction.** Lowest-friction item on the whole roadmap: self-supervised from the existing `data/cleaned/train.jsonl` corpus via synthetic corruption, no external data sourcing or license-vetting needed (unlike translation, item 5). It's also the next real data point on the open question underneath this whole roadmap — does fine-tuning this architecture on a new task actually work — this time on a *generation* task rather than classification, which is the harder and more informative test.
